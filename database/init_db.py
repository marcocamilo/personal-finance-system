...
def seed_budget_templates():
    ...
    couples_id = conn.execute(
        "SELECT id FROM budget_templates WHERE name = 'Couples'"
    ).fetchone()[0]
    working_id = conn.execute(
        "SELECT id FROM budget_templates WHERE name = 'Working Couples'"
    ).fetchone()[0]
